...
export default class Login extends Component {
	...
	loginDialog() {
		...
		el.querySelector<HTMLInputElement>("#loginHomeserver")!.value =
			localStorage.getItem("homeserver") || "https://matrix.org"

		el.querySelector("#loginHSSubmit")!.addEventListener("click", async () => {
			const homeserver = (
				el.querySelector("#loginHomeserver") as HTMLInputElement
			).value
			...
			el.querySelector<HTMLInputElement>("#loginHomeserver")!.style.display =
				"none"
			el.querySelector<HTMLButtonElement>("#loginHSSubmit")!.style.display =
				"none"
			el.querySelector<HTMLInputElement>("#loginUsername")!.style.display =
				"block"
			el.querySelector<HTMLInputElement>("#loginPassword")!.style.display =
				"block"
			el.querySelector<HTMLButtonElement>("#loginSubmit")!.style.display =
				"block"
		})

		el.querySelector<HTMLButtonElement>("#loginSubmit")!.addEventListener(
			"click",
			async () => {
				const username =
					el.querySelector<HTMLInputElement>("#loginUsername")!.value
				const password =
					el.querySelector<HTMLInputElement>("#loginPassword")!.value
				try {
	...
	registerDialog() {
		...
		el.querySelector<HTMLInputElement>("#registerHomeserver")!.value =
			localStorage.getItem("homeserver") || "https://matrix.org"

		el.querySelector("#registerHSSubmit")!.addEventListener("click", () => {
			const homeserver = (
				el.querySelector("#registerHomeserver") as HTMLInputElement
			).value
			if (homeserver != localStorage.getItem("homeserver")) {
				localStorage.setItem("homeserver", homeserver)
				location.reload()
			...
			el.querySelector<HTMLInputElement>("#registerHomeserver")!.style.display =
				"none"
			el.querySelector<HTMLButtonElement>("#registerHSSubmit")!.style.display =
				"none"
			el.querySelector<HTMLInputElement>("#registerUsername")!.style.display =
				"block"
			el.querySelector<HTMLInputElement>("#registerPassword")!.style.display =
				"block"
			el.querySelector<HTMLButtonElement>("#registerSubmit")!.style.display =
				"block"
		})

		el.querySelector<HTMLButtonElement>("#registerSubmit")!.addEventListener(
			"click",
			async () => {
				const username =
					el.querySelector<HTMLInputElement>("#registerUsername")!.value
				const password =
					el.querySelector<HTMLInputElement>("#registerPassword")!.value
				try {
